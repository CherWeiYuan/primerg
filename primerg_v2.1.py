...
pam = ''                              #e.g. "NGG", let pam be '' if pamless **********************************
gRNA_len = 20
...
valid_amplicon_size = 500              # Size of amplicon produced by unspecific primers to be considered valid amplicon
NGS_amplicon_size = 150                # Length of amplicon of EACH primer in paired-end sequencing; default is set at 150 for iSeq

#  Primerg
output_excel = "testing.xlsx"          #if an excel name is specified (e.g. filename.xlsx), output will be an excel sheet. If False, output will be a pandas dataframe.
                    ...
                    if hsp.match.count(' ') <= total_mismatch:
                        if hsp.strand[1] == 'Plus':
                            if hsp.match[-5:].count('|') >= three_prime_match:
                                pos_binding[0] += [hsp.query_end]     # store position at 3' end of forward primer
                        elif hsp.strand[1] == 'Minus':
                            if hsp.match[0:4].count('|') >= three_prime_match:
                                pos_binding[1] += [hsp.query_start]   # store position at 3' end of reverse primer
